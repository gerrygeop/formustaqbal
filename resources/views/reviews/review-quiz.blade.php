										...
										<div class="py-10">
												...
												<div class="grid grid-cols-1 gap-4" @if ($question->is_choice_rtl) dir="rtl" @endif>
													@foreach ($question->choices as $choice)
														<div class="text-lg flex items-center gap-x-4">
															...
															@if ($answers[$question->id]->answer == $choice->id && $choice->is_correct)
																<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
																	stroke="currentColor" class="w-7 h-7 text-green-600">
																	<path stroke-linecap="round" stroke-linejoin="round"
																		d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
																</svg>
															@elseif ($answers[$question->id]->answer == $choice->id && !$choice->is_correct)
																<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
															...
															<span @class([
																'font-semibold' => $answers[$question->id]->answer == $choice->id,
															])>
																{{ $choice->choice }}
															</span>
														</div>
											...
											@elseif ($question->type == 2 || $question->type == 3)
												<div class="bg-gray-100 border p-4">
													<p>{{ $answers[$question->id]->answer }}</p>
												</div>
											@elseif ($question->type == 4)
												@if (str($answers[$question->id]->answer)->endsWith('.mp3'))
													<audio controls class="bg-yellow-400 w-full">
														<source src="{{ asset('storage/' . $answers[$question->id]->answer) }}" type="audio/mpeg">
														Your browser does not support the audio element.
													</audio>
												@else
													<img src="{{ asset('storage/' . $answers[$question->id]->answer) }}" alt="Image"
