	...
	<section class="fixed inset-0 overflow-y-auto z-50 bg-gray-50 dark:bg-gray-900">
		<div class="sticky top-0 bg-white dark:bg-gray-800 py-3 md:py-4 px-4 md:px-8 z-[51] shadow">
			<div class="flex items-center justify-between">
				<a href="{{ url('/dapur') }}" class="flex-1 flex items-center gap-x-2 text-gray-600 dark:text-gray-50">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
						class="w-6 h-6">
						<path stroke-linecap="round" stroke-linejoin="round" d="M6.75 15.75L3 12m0 0l3.75-3.75M3 12h18" />
					</svg>
					<p class="font-semibold text-base hidden md:block">Dashboard</p>
				</a>
			</div>
		</div>
				...
				<div class="py-6 mb-20">
					...
					<div class="my-6">
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
							...
							<div class="bg-white rounded-lg shadow p-6">
								<h3 class="text-2xl text-yellow-600 font-semibold">{{ $usersNoAssessment }} / {{ $total_users }}</h3>
								<p class="text-sm text-slate-700">Tidak mengambil Placement Test</p>
								<a href="{{ route('dapur.latte', $assessment) }}"
									class="mt-1 text-sm text-blue-500 hover:underline flex items-center gap-x-2">
									<span>Detail</span>
					...
					@livewire('list-users-espresso', ['assessment' => $assessment])
